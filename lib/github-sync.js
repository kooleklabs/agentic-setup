...
const { createImplBranch, commitImplChanges, remoteBranchExists } = require('./gh-impl-branch.js');
...
async function runExecute(ctx) {
  ...
  // 3. Pre-flight + create impl branch
  let branchInfo;
  try {
  ...
  printExecutePlan({ ...ctx, branch: branchInfo.branch });

  if (ctx.dryRun) {
    console.log('(dry-run — no SDK call, no commit. Cost cap would be $' +
      ctx.maxCostUsd.toFixed(2) + ')');
    return {
      ok: true,
      dryRun: true,
      impl: { branch: branchInfo.branch, slug: ctx.implBranchName },
    };
  }
  ...
  // 7. Comment on Issue (unless opted out)
  if (!ctx.noComment) {
    const commentBody = sha
      ? `Implementation pass complete on local branch \`${branchInfo.branch}\` (commit \`${sha.slice(0, 7)}\`). Review the diff, then open a draft PR when ready.`
      : `Implementation pass against \`${branchInfo.branch}\` produced no file changes — nothing to commit. Inspect the prompt / model output before re-running.`;
    commentOnIssue({
      owner: ctx.owner,
      repo: ctx.repo,
  ...
  printExecuteBanner({ branch: branchInfo.branch, sha, impl });
...
function printExecuteBanner({ branch, sha, impl }) {
  console.log('');
  if (sha) {
