...
async function runGithubSync(opts = {}) {
  ...
  try {
    ...
    if (projectNumber) {
      logProjectAttach('plan PR', attachToProject({
        owner, projectNumber, contentUrl: pr.url, statusOptionName: 'In Progress',
      }));
    }
  ...
  if (ctx.openPr && sha) {
    ...
    try {
      ...
      if (ctx.projectNumber) {
        logProjectAttach('impl PR', attachToProject({
          owner: ctx.owner,
          projectNumber: ctx.projectNumber,
          contentUrl: pr.url,
          statusOptionName: 'In Review',
        }));
      }
    } catch (err) {
...
function logProjectAttach(label, r) {
  if (r.ok && r.statusSet) {
    console.log(`  ↳ project: ${label} added + Status set`);
  } else if (r.ok) {
