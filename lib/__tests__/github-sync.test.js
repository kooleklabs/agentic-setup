...
jest.mock('../self-review.js');
jest.mock('../gh-project.js');
...
const selfReviewMod = require('../self-review.js');
...
beforeEach(() => {
  ...
  selfReviewMod.runSelfReview.mockReturnValue({
    checks: [
      { name: 'test', command: 'npm test', status: 'pass', exitCode: 0, duration: 1000 },
    ],
    hasFailures: false,
  });
  ghProject.attachToProject.mockReturnValue({ ok: true, itemId: 'PVTI_x', statusSet: true });
});
afterEach(() => {
...
describe('runGithubSync — plan mode --project', () => {
  test('--project attaches the plan PR to project with Status "In Progress"', async () => {
    await runGithubSync({ cwd: tmp, issueNumber: 42, autoApprove: true, projectNumber: 1 });
    expect(ghProject.attachToProject).toHaveBeenCalledWith(
      expect.objectContaining({ projectNumber: 1, statusOptionName: 'In Progress' })
    );
  });

  test('no --project: attachToProject is not called', async () => {
    await runGithubSync({ cwd: tmp, issueNumber: 42, autoApprove: true });
    expect(ghProject.attachToProject).not.toHaveBeenCalled();
  });
});
...
describe('runGithubSync — --execute mode', () => {
  ...
  test('--open-pr runs self-review before opening PR; result is forwarded to openImplPR', async () => {
    await runGithubSync({
      cwd: tmp, issueNumber: 42, execute: true, autoApprove: true, openPr: true,
    });
    expect(selfReviewMod.runSelfReview).toHaveBeenCalledTimes(1);
    // runSelfReview ordered before openImplPR
    expect(selfReviewMod.runSelfReview.mock.invocationCallOrder[0])
      .toBeLessThan(implPR.openImplPR.mock.invocationCallOrder[0]);
    const prCallArgs = implPR.openImplPR.mock.calls[0][0];
    expect(prCallArgs.selfReview).toEqual(
      expect.objectContaining({ hasFailures: false })
    );
  });

  test('self-review is NOT run without --open-pr (local-branch workflow is unchanged)', async () => {
    await runGithubSync({
      cwd: tmp, issueNumber: 42, execute: true, autoApprove: true,
    });
    expect(selfReviewMod.runSelfReview).not.toHaveBeenCalled();
  });

  test('--project: attaches impl PR to project with Status "In Review"', async () => {
    await runGithubSync({
      cwd: tmp, issueNumber: 42, execute: true, autoApprove: true, openPr: true, projectNumber: 1,
    });
    expect(ghProject.attachToProject).toHaveBeenCalledWith(
      expect.objectContaining({ projectNumber: 1, statusOptionName: 'In Review' })
    );
  });

  test('--project is NOT called without --open-pr in execute mode (no PR to attach)', async () => {
    await runGithubSync({
      cwd: tmp, issueNumber: 42, execute: true, autoApprove: true, projectNumber: 1,
    });
    expect(ghProject.attachToProject).not.toHaveBeenCalled();
  });

  test('self-review failures do not block PR creation (warning in body only)', async () => {
    selfReviewMod.runSelfReview.mockReturnValue({
      checks: [
        { name: 'test', command: 'npm test', status: 'fail', exitCode: 1, duration: 1000 },
      ],
      hasFailures: true,
    });
    const r = await runGithubSync({
    ...
    expect(r.ok).toBe(true);
    expect(implPR.openImplPR).toHaveBeenCalledTimes(1);
    expect(r.impl.selfReview.hasFailures).toBe(true);
  });
});
