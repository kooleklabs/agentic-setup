...
describe('commentOnIssue', () => {
  test('POSTs to repos/:owner/:repo/issues/:n/comments with body field', () => {
    cp.spawnSync.mockReturnValueOnce({ status: 0, stdout: '{"id": 1}', stderr: '' });
    commentOnIssue({ owner: 'o', repo: 'r', number: 5, body: 'Plan PR: #7' });
    const [cmd, args] = cp.spawnSync.mock.calls[0];
    expect(cmd).toBe('gh');
    expect(args).toContain('api');
    expect(args).toContain('POST');
    expect(args.some((a) => a.includes('repos/o/r/issues/5/comments'))).toBe(true);
    expect(args.some((a) => a.startsWith('body='))).toBe(true);
  });

  test('throws on error', () => {
    cp.spawnSync.mockReturnValueOnce({ status: 1, stdout: '', stderr: 'Forbidden' });
    expect(() => commentOnIssue({ owner: 'o', repo: 'r', number: 99, body: 'x' }))
      .toThrow(/Forbidden/);
  });
});
