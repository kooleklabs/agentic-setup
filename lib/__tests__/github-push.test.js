...
describe('runGithubPush', () => {
  ...
  test('--force ignores existing FEATURE markers and creates everything fresh', async () => {
    issues.listOpenIssuesWithMarkers.mockReturnValue([
      { number: 7, title: 'Low stock alert', markers: [{ type: 'feature', slug: 'low-stock-alert' }] },
    ]);
  ...
  test('--force still reuses an existing milestone (does not duplicate the singleton)', async () => {
    issues.listMilestones.mockReturnValue([{ number: 42, title: 'MyApp v1.0' }]);
    await runGithubPush({ cwd: '.', autoApprove: true, force: true });
    expect(issues.createMilestone).not.toHaveBeenCalled();
  });

  test('--force still reuses an existing umbrella Issue (does not duplicate the singleton)', async () => {
    issues.listOpenIssuesWithMarkers.mockReturnValue([
      { number: 11, title: 'Umbrella', markers: [{ type: 'umbrella', slug: 'myapp' }] },
    ]);
    await runGithubPush({ cwd: '.', autoApprove: true, force: true });
    // Umbrella should NOT be created (singleton). Only 2 feature Issues.
    expect(issues.createIssue).toHaveBeenCalledTimes(2);
    // Umbrella body gets refreshed in place with new feature numbers.
