...
describe('parseArgs', () => {
  ...
  test('--project captures a positive integer', () => {
    expect(parseArgs(['--issue', '1', '--project', '7']).projectNumber).toBe(7);
  });

  test('--project rejects non-numeric or non-positive', () => {
    expect(() => parseArgs(['--issue', '1', '--project', 'abc'])).toThrow(/--project/);
    expect(() => parseArgs(['--issue', '1', '--project', '-1'])).toThrow(/--project/);
  });
