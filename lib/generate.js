...
// Note: architect-gate is lazy-loaded inside main() so that --help and
// other non-generate paths don't require runtime deps like js-yaml.
...
async function main(argv = process.argv.slice(3)) {
  ...
  // Lazy-load the gate module so that --help and other non-gate paths
  // don't have to resolve the js-yaml runtime dependency.
