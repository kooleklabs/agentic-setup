...
function parseArgs(argv) {
  const out = { mode: 'interactive', interactive: false };
...
function sdkOptions(abort, isContinuation) {
  return {
    cwd: process.cwd(),
    allowedTools: [
      'Write',
    ...
    // settingSources defaults to [] so user settings layers are not
    // loaded. But the SDK spawns claude-code as a subprocess and that
    // subprocess can still auto-discover installed plugin hooks, which
    // may veto Write/Edit tool calls based on substring matching in
    // content. We pass the security-guidance plugin's documented
    // opt-out env var here so the hook no-ops cleanly if it does load.
 ...
 */
async function runOneTurn({ query, prompt, abort, renderer, isContinuation }) {
  ...
  try {
    for await (const message of query({
      prompt,
      options: sdkOptions(abort, isContinuation),
    })) {
...
async function callAgentSdk(initialPrompt, opts = {}) {
  const { interactive = false, maxTurns = 5 } = opts;
  const { query } = loadSdk();
...
async function main(argv = process.argv.slice(3)) {
  ...
  try {
    const result = await callAgentSdk(prompt, { interactive: args.interactive });
